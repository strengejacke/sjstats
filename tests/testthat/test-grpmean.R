if (require("testthat") && require("sjstats") && require("dplyr")) {
  ...
  test_that("means_by_group", {
    expect_equal(means_by_group(efc, c12hour, e42dep), c(9.90909, 17.53778, 34.51961, 75.90132, 42.4384), tolerance = 1e-3)
  })

  test_that("means_by_group, weighting", {
    w <- "weight"
    expect_equal(means_by_group(efc, c12hour, e42dep, weights = weight), c(9.43932, 17.28629, 35.16486, 79.23457, 43.0544), tolerance = 1e-3)
    expect_equal(means_by_group(efc, c12hour, e42dep, weights = "weight"), c(9.43932, 17.28629, 35.16486, 79.23457, 43.0544), tolerance = 1e-3)
    expect_equal(means_by_group(efc, c12hour, e42dep, weights = w), c(9.43932, 17.28629, 35.16486, 79.23457, 43.0544), tolerance = 1e-3)
  })
