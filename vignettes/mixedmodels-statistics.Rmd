...
Further variance components can be calculated using `adjusted = TRUE`. In this case, `re_var()` returns the variance attributable to the fixed effects, the variance of the random effects, the variance due to additive dispersion and the distribution-specific variance. The residual variance is the sum of the dispersion and distribution-specific variance (see _Nakagawa et al. (2017)_ and _Johnson et al. (2014)_):
