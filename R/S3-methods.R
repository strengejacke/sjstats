...
#' @export
rhat.stanreg <- function(object, pars, ...) {
  # get stan summary, with rhat stats
  xds <- object$stan_summary %>%
    as.data.frame() %>%
  ...
  if (!missing(pars) && !is.null(pars)) {
    if (is.numeric(pars))
      ...
      out <- xds[["Rhat"]][match(pars, xds[["rowname"]])]
  }
...
#' @export
neff_ratio <- function(object, pars = NULL, ...) {
  UseMethod("neff_ratio")
}


#' @export
rhat.stanreg <- function(object, pars = NULL, ...) {
  stan_stats(object, pars, "Rhat", ...)
}


#' @export
neff_ratio.stanreg <- function(object, pars = NULL, ...) {
  stan_stats(object, pars, "n_eff", ...)
}


#' @importFrom tibble rownames_to_column
stan_stats <- function(object, pars, stat, ...) {
  # get stan summary, with rhat stats
  xds <- object$stan_summary %>%
    as.data.frame() %>%
    tibble::rownames_to_column()
  ...
  if (!missing(pars) && !is.null(pars)) {
    if (is.numeric(pars))
      ...
      out <- xds[[stat]][match(pars, xds[["rowname"]])]
  }
